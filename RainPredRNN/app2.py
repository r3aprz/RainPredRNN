...
# Aggiungi questa sezione per il parallelismo multi-GPU
...
for epoch in range(epochs):
    ...
    total_loss = 0.0  # Accumulatore per la loss dell'epoca
    num_batches = 0    # Contatore batch
    ...
    for batch_idx, (inputs, targets) in enumerate(dataloader):
        ...
        with autocast():
            ...
            outputs = F.interpolate(
                outputs, 
                size=(targets.size(3), targets.size(4)), 
                mode='bicubic', 
                align_corners=False
            )
        ...
        # Backpropagation con precisione mista
    ...
    print(f'Loss media epoca: {avg_loss:.4f}')
...
output_dir = "/home/f.demicco/RainPredRNN/RainPredRNN/output_predictions"
os.makedirs(output_dir, exist_ok=True)

for i in range(predicted.shape[0]):
    output_tiff_path = os.path.join(output_dir, f"prediction_pj{i+1}.tiff")
